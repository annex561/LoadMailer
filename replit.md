...
**TASKMAGIC INTEGRATION**: Complete TaskMagic automation provided with full DAT scraping script based on user's login code. Comprehensive automation handles DAT login with 2FA, applies filters for box trucks/sprinter vans in southeastern states, extracts load data using multiple detection methods, and sends up to 50 loads per run to LoadMaster webhook. Script includes rate limiting, error handling, and equipment type mapping. LoadMaster processes scraped data and dispatches to drivers with automatic customer creation, load validation, and immediate Telegram driver notifications. User has complete automation script ready for TaskMagic import with DAT credentials (dispatch@lampslogistics.com/Anonymous#56111) and webhook configuration.
