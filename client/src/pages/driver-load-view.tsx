    ...
    <div className="min-h-screen bg-slate-100 p-4 pb-20">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">TRAQ IQ Logistics</p>
        </div>
        <Badge className="bg-blue-600 text-lg px-3 py-1">{(load.status || 'pending').toUpperCase()}</Badge>
      </div>
      ...
      <Card className="mb-4 shadow-sm border-0">
        <CardContent className="pt-6 space-y-6">
          <div className="relative pl-8 border-l-2 border-emerald-500">
            ...
            <p className="text-lg font-bold text-slate-900">{load.originCity || load.pickupAddress || 'TBD'}</p>
            <p className="text-sm text-slate-600">{load.pickupDate ? new Date(load.pickupDate).toLocaleDateString() : 'TBD'}</p>
          ...
          <div className="relative pl-8 border-l-2 border-red-500">
            ...
            <p className="text-lg font-bold text-slate-900">{load.destCity || load.deliveryAddress || 'TBD'}</p>
            <p className="text-sm text-slate-600">{load.deliveryDate ? new Date(load.deliveryDate).toLocaleDateString() : 'TBD'}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="mb-6 shadow-sm border-0">
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          <div className="bg-slate-50 p-3 rounded-lg">
            ...
            <p className="font-bold text-slate-800">{load.weight ? `${load.weight.toLocaleString()} lbs` : 'TBD'}</p>
          </div>
          <div className="bg-slate-50 p-3 rounded-lg">
            <p className="text-xs text-slate-400">Rate</p>
            <p className="font-bold text-slate-800">${load.rate || 0}</p>
          </div>
        </CardContent>
      </Card>
