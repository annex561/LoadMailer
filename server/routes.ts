...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  registerLoadsAuditRoutes(app);

  // Ratecon intake routes (PDF upload + manual entry)
  registerRateconIntakeRoutes(app);
  registerDriverConfirmationRoutes(app);

  // Universal e-signature integration via DocuSeal (hosted OR self-hosted).
  // Powers OO lease packets, driver onboarding signatures, and any one-off doc send.
  ...
  app.patch('/api/drivers/:id/pay', async (req, res) => {
    try {
      ...
      const { eq } = await import('drizzle-orm');
      const { payType, payRate, weeklyFuelCost, weeklyInsuranceCost } = req.body || {};
      ...
      const patch: any = { payType, payRate: rate };
      if (weeklyFuelCost !== undefined) {
        const fuel = Number(weeklyFuelCost);
        if (!isFinite(fuel) || fuel < 0) return res.status(400).json({ ok: false, error: 'weeklyFuelCost must be a non-negative number' });
        patch.weeklyFuelCost = fuel;
      }
      if (weeklyInsuranceCost !== undefined) {
        const ins = Number(weeklyInsuranceCost);
        if (!isFinite(ins) || ins < 0) return res.status(400).json({ ok: false, error: 'weeklyInsuranceCost must be a non-negative number' });
        patch.weeklyInsuranceCost = ins;
      }
...
<p style="color:#666;font-size:13px;margin-top:32px;">Reply <strong>YES</strong> to your dispatch SMS to confirm the load.</p>
  ...
  <td style="text-align:right"><b>$${l.pay.toFixed(2)}</b></td>
      ...
      const fuelCost = settlement?.fuelCost ?? +(driver.weeklyFuelCost ?? 0);
      const insuranceCost = settlement?.insuranceCost ?? +(driver.weeklyInsuranceCost ?? 0);
      const netPay = settlement?.netPay ?? -(fuelCost + insuranceCost);
...
<style>
  ...
  .breakdown{background:#fafafa;border:1px solid #e5e7eb;border-radius:8px;padding:12px 16px;margin:12px 0;font-size:14px}
  .breakdown .row{display:flex;justify-content:space-between;padding:4px 0}
  .breakdown .row.total-row{border-top:1px solid #e5e7eb;margin-top:6px;padding-top:8px;font-weight:700}
  .breakdown .ded{color:#dc2626}
  ...
  <div class="meta">Pay rule: ${driver.payType || 'percent'} @ ${driver.payRate ?? 80}${
        (driver.payType || 'percent') === 'percent' ? '%' : '$'
      }</div>
  <div class="breakdown">
    <div class="row"><span>Gross revenue</span><span>$${totalRev.toFixed(2)}</span></div>
    <div class="row"><span>Your share (${driver.payRate ?? 80}${(driver.payType || 'percent') === 'percent' ? '%' : ''})</span><span>$${grossPay.toFixed(2)}</span></div>
    <div class="row ded"><span>− Fuel</span><span>-$${fuelCost.toFixed(2)}</span></div>
    <div class="row ded"><span>− Insurance</span><span>-$${insuranceCost.toFixed(2)}</span></div>
    <div class="row total-row"><span>Net take-home</span><span>$${netPay.toFixed(2)}</span></div>
  </div>
  <div class="total">
