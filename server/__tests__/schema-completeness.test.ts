...
function extractDriverColumnsFromSchema(): string[] {
  ...
  const tableMatch = src.match(/export const drivers = pgTable\("drivers", \{([\s\S]*?)\n\}(?:,|\))/);
  if (!tableMatch) {
    throw new Error("Could not locate `drivers` pgTable block in shared/schema.ts");
  }
  const block = tableMatch[1];
  const columnRegex = /(?:varchar|text|integer|boolean|timestamp|real|jsonb|serial|bigint|date|numeric|pgEnum)\s*\(\s*"([a-z_][a-z0-9_]*)"/g;
...
function extractDriverColumnsFromEnsureSchema(): string[] {
  ...
  // Grab everything inside the first `const columns: [string, string][] = [ ... ];` block.
  const block = src.match(/const columns:\s*\[string,\s*string\]\[\]\s*=\s*\[([\s\S]*?)\n\s*\];/);
  if (!block) {
    throw new Error("Could not locate `const columns` array in server/ensure-schema.ts");
  }
 ...
 */
const ORIGINAL_TABLE_COLUMNS = new Set([
  "id",
  "name",
  "email",
  "phone",
...
describe("schema completeness — drivers table", () => {
  it("every column added to drivers schema.ts is registered in ensure-schema.ts", () => {
    ...
    const missing = schemaCols.filter(
      (c) => !ORIGINAL_TABLE_COLUMNS.has(c) && !ensureSet.has(c),
    );
